  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { error } = await supabase
        ...
        .eq("id", row.id); // Identifying the row by the int8 ID
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            ...
            <input className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-blue-500/20 outline-none transition" 
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="number" className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-blue-500/20 outline-none transition" 
          ...
          <div>
            ...
            <select className="w-full p-2 border rounded-lg bg-white focus:ring-2 focus:ring-blue-500/20 outline-none transition"
          ...
          <div>
            ...
            <textarea rows={2} className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-blue-500/20 outline-none transition" 
...
const InventoryList = () => {
  ...
  const loadInitialData = async () => {
    setLoading(true);
    try {
      // ORDER BY ID DESCENDING (Highest ID first)
      // This ensures stability even if created_at times are identical
  ...
  const loadMoreData = async () => {
    ...
    try {
      ...
      // ORDER BY ID DESCENDING HERE TOO
  ...
  const handleDelete = async (id) => {
    if (!window.confirm(`Are you sure you want to delete Record #${id} permanently?`)) return;
    try {
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex gap-3">
             <button onClick={loadInitialData} className="p-2.5 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 text-slate-600 transition" title="Refresh">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden min-h-[400px] flex flex-col">
          {loading && (
            <div className="flex flex-col items-center justify-center flex-1 space-y-3 p-12">
                    ...
                    <tr key={item.id} className="hover:bg-slate-50/80 transition-colors group">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex gap-2">
                          {item.receipt_url ? (
                            <a href={item.receipt_url} target="_blank" rel="noreferrer" className="p-1.5 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 transition">
                              <FileText size={16} />
                            </a>
                          ) : <span className="p-1.5 bg-slate-50 text-slate-300 rounded-lg"><FileText size={16} /></span>}
                          {item.photo_urls && item.photo_urls.length > 0 ? (
                            <div className="p-1.5 bg-purple-50 text-purple-600 rounded-lg relative">
                              <ImageIcon size={16} />
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => setEditingItem(item)} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition"><Edit2 size={16} /></button>
          ...
          {/* Load More */}
          {!loading && hasMore && filteredData.length > 0 && !searchTerm && (
             <div className="p-4 border-t border-slate-100 bg-slate-50/50 flex justify-center">
                <button onClick={loadMoreData} disabled={loadingMore} className="flex items-center gap-2 px-6 py-2 bg-white border border-slate-200 shadow-sm text-slate-600 text-sm font-medium rounded-full hover:bg-slate-50 disabled:opacity-50 transition">
                  {loadingMore ? <><RefreshCw size={14} className="animate-spin" /> Loading...</> : <><ChevronDown size={14} /> Load Older Records</>}
                </button>
             </div>
          )}
        </div>
